...
import { linkSchema } from '@/extension/samples/extension-schema';
import LinkWidget from '@/extension/samples/link/link-widget';
import { linkTemplateGenerator } from '@/extension/samples/extension-sfc-generator';
...
export const loadExtension = function () {
   ...
   * 5. 加载完毕。
   */
  addCustomWidgetSchema(linkSchema); //加载组件Json Schema
  /* -------------------------------------------------- */
  Vue.component(LinkWidget.name, LinkWidget); //注册组件
  /* -------------------------------------------------- */
  PERegister.registerCPEditor(
    'link-title',
    'link-title-editor',
    PEFactory.createInputTextEditor('title', 'extension.setting.linkTitle')
  );
  ...
  Vue.component(
    'link-type-editor',
    PEFactory.createSelectEditor('type', 'extension.setting.linkType', { optionItems: linkTypeOptions })
  );

  PERegister.registerCPEditor(
    'link-underline',
    'link-underline-editor',
    PEFactory.createBooleanEditor('underline', 'extension.setting.underline')
  );

  PERegister.registerCPEditor(
    'link-href',
    'link-href-editor',
    PEFactory.createInputTextEditor('href', 'extension.setting.href')
  );

  PERegister.registerCPEditor(
    'link-icon',
    'link-icon-editor',
    PEFactory.createInputTextEditor('icon', 'extension.setting.icon')
  );
  ...
  let linkTargetOptions = [
    ...
    { label: '_top', value: '_parent' },
  ];

  PERegister.registerCPEditor(
    'link-target',
    'link-target-editor',
    PEFactory.createSelectEditor('target', 'extension.setting.linkTarget', { optionItems: linkTargetOptions })
  );

  PERegister.registerEPEditor('link-onClick', 'link-onClick-editor', PEFactory.createEventHandlerEditor('onClick', []));
  /* -------------------------------------------------- */
  registerFWGenerator('link', linkTemplateGenerator); //注册字段组件的代码生成器
  /* -------------------------------------------------- */
  /* 超链接组件加载完毕 end */
};
